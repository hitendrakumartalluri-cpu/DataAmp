...
if [[ ! -f "$ROOT/docker-compose.lab.yml" ]]; then
  if [[ -f "./docker-compose.lab.yml" ]]; then
    ROOT="$(pwd)"
  elif [[ -f "../docker-compose.lab.yml" ]]; then
    ROOT="$(cd .. && pwd)"
  fi
fi
cd "$ROOT"

ENV_FILE="${ENV_FILE:-.env.lab}"
...
kafka_total(){ docker compose --env-file "$ENV_FILE" -f docker-compose.lab.yml exec -T kafka /opt/kafka/bin/kafka-get-offsets.sh --bootstrap-server kafka:9092 --topic "$1" 2>/dev/null | awk -F: '{sum += $3} END {print sum+0}'; }
consumer_lag(){ docker compose --env-file "$ENV_FILE" -f docker-compose.lab.yml exec -T kafka /opt/kafka/bin/kafka-consumer-groups.sh --bootstrap-server kafka:9092 --describe --group "$1" 2>/dev/null | awk 'NR>1 && $6 ~ /^[0-9]+$/ {sum += $6} END {print sum+0}'; }
failed_event_count(){
  local body
...
readiness_check(){
  ...
  docker compose --env-file "$ENV_FILE" -f docker-compose.lab.yml exec -T amp python - <<'PY'
import os,boto3
